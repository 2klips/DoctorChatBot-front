...
### 기획 단계
   ...
   * **프로젝트 기획:** 결정된 아이디어를 바탕으로, 전체적인 프로젝트의 기획 및 페이지 구성등을 설계했습니다.
     
   * **디자인:** 프로젝트의 전체 디자인 ( UI, Logo ) 작업을 진행하였습니다.
   
   * **데이터 탐색:** 데이터 탐색 및 분석을 통해 프로젝트에 적합한 데이터셋을 선정 했습니다

   * * **학습 과정 분석:** 모델의 학습 과정을 분석하고 결과를 평가하여 성능 향상 방안을 모색했습니다. [ TensorBoard ]

### 모델

   * **데이터셋 전처리:** 데이터셋의 전처리를 진행하여 모델 학습에 적합한 형태로 데이터를 준비했습니다.
   
   * **모델 파인튜닝 및 학습:** 프로젝트 목표에 적합한 모델과 데이터셋을 선정하고, 모델 분석 및 사용가능한 하드웨어를 바탕으로 최적의 파인튜닝을 위한 설계 및 파인튜닝 학습을 진행하였습니다.
   
   * **모델 학습 최적화:** QLora, Transformer, bnb 기법을 통해 학습을 위한 메모리 최적화를 진행했습니다.
